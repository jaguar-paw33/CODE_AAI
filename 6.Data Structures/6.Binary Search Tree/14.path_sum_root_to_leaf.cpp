...
void sum_path_to_leaf(binaryTreeNode<int>* root, int sum, vector<int> & v) {
	if (!root) {
		return;
	}
	...
	sum_path_to_leaf(root->left, sum - root->data, v);
	sum_path_to_leaf(root->right, sum - root->data, v);
	v.pop_back();
}
